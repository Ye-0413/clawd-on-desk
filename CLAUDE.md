...
npm run build          # electron-builder 打包 Windows NSIS（本地若需禁止发布，加：--publish never）
npm run build:mac      # electron-builder 打包 macOS DMG（同上）
npm run build:linux    # electron-builder 打包 Linux AppImage + deb（同上）
npm run build:all      # 同时打包 Windows + macOS + Linux（同上）
# 示例（本地打包且不触发 publish）：npx electron-builder --win --publish never
# CI：.github/workflows/build.yml 使用 npx electron-builder --<platform> --publish never
...
- **启动恢复**：桌宠在 agent 会话中途启动时，`detectRunningAgentProcesses()` 会检测已运行的 CLI/agent 相关进程（不含 Cursor IDE）并激活 `startupRecoveryActive` 标志，抑制 idle→sleep 序列，保持 idle-follow 等待 hook 到来；若未检测到进程则保持 idle 直到下一个 hook 事件触发
